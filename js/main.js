(function(){
    ...
    // ========== SECURITY (DISEDERHANAKAN AGAR TIDAK CONFLICT) ==========
    ...
    function showSecurityAlert(m) { 
        ...
        setTimeout(() => { if(securityAlert) securityAlert.style.display = 'none'; }, 2000); 
    }
    
    // Basic security - tanpa debugger loop yang bikin lag
    ...
    if (overlay) {
        ...
        if (closeUpdateBtn) closeUpdateBtn.addEventListener('click', hideOverlay);
    ...
    document.querySelectorAll('.amount-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (donateAmount) donateAmount.value = this.dataset.amount;
        });
    });
    ...
    if (generateQRBtn) {
        generateQRBtn.addEventListener('click', async function() {
            ...
            this.disabled = false;
            this.innerHTML = '<i class="fas fa-qrcode"></i> Buat QRIS';
    ...
    if (checkPaymentBtn) {
        checkPaymentBtn.addEventListener('click', async function() {
            ...
            this.disabled = true;
            this.innerHTML = '<span class="spinner"></span> Mengecek...';
            const result = await checkPaymentStatusAPI(currentTransactionId);
            this.disabled = false;
            this.innerHTML = '<i class="fas fa-sync-alt"></i> Cek Status';
    ...
    function showProgress(s, t = '', p = 0) { 
        ...
        if (s) { 
            if (progText) progText.textContent = t; 
            if (progFill) progFill.style.width = Math.min(p, 100) + '%'; 
        } 
    }
    ...
    async function processVideo(file) {
        ...
        if (!ffmpegReady) {
            showProgress(true, 'Mode Fallback...', 50);
        ...
        mediaItems.forEach((it, i) => {
            ...
            h += `<div class="preview-card">
                ...
                <div class="action-row">
                    <button class="icon-btn download-btn" data-i="${i}"><i class="fas fa-download"></i></button>
                    <button class="icon-btn remove-btn" data-i="${i}"><i class="fas fa-trash"></i></button>
                </div>
            </div>`;
        });
    ...
    async function processFile(file, cat) {
        if (file.size > (cat === 'video' ? 100 : 50) * 1024 * 1024) {
            alert('File terlalu besar! Maks ' + (cat === 'video' ? '100MB' : '50MB'));
            return;
        }
